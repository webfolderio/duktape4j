...
void queueDuktapeException(JNIEnv* env, const std::string& message) {
  const jclass exceptionClass = env->FindClass("io/webfolder/ducktape4j/DuktapeException");
...
void checkRethrowDuktapeError(JNIEnv* env, duk_context* ctx) {
  ...
  jthrowable globalEx = (jthrowable) env->NewGlobalRef(env->ExceptionOccurred());
  duk_push_pointer(ctx, globalEx);
  env->ExceptionClear();
...
void queueJavaExceptionForDuktapeError(JNIEnv *env, duk_context *ctx) {
  jclass exceptionClass = env->FindClass("io/webfolder/ducktape4j/DuktapeException");
